...
class Cfg:
  ...
  actor_lr: float = 3e-4
  critic_lr: float = 1e-4            # refined: slower critic updates for Q-learning stability
  alpha_lr: float = 3e-4
  max_grad_norm: float = 5.0        # refined: gradient clipping for stable gradient descent
...
class CNNEncoder(nn.Module):
  def __init__(self):
      ...
      self.out_dim = n
      # refined architecture: LayerNorm on encoder features for stability
...
def main():
  ...
  act_opt = optim.Adam(actor.parameters(), lr=cfg.actor_lr)
  q1_opt = optim.Adam(q1.parameters(), lr=cfg.critic_lr)
  q2_opt = optim.Adam(q2.parameters(), lr=cfg.critic_lr)
      ...
      if len(rb) >= cfg.batch_size:
          for _ in range(cfg.updates_per_step):
              ...
              q2_pred = q2(o_t, a_tanh)
                             # refined Q-learning loss: Huber (smooth L1) is more robust than MSE for TD errors
              q1_loss = nn.functional.smooth_l1_loss(q1_pred, y)
              q2_loss = nn.functional.smooth_l1_loss(q2_pred, y)


              q1_opt.zero_grad()
              q1_loss.backward()
              nn.utils.clip_grad_norm_(q1.parameters(), cfg.max_grad_norm)
              q1_opt.step()


              q2_opt.zero_grad()
              q2_loss.backward()
              nn.utils.clip_grad_norm_(q2.parameters(), cfg.max_grad_norm)
              q2_opt.step()
              ...
              if global_step % cfg.policy_delay == 0:
                  ...
                  alpha_opt.zero_grad()
                  alpha_loss.backward()
                  nn.utils.clip_grad_norm_([log_alpha], cfg.max_grad_norm)
                  alpha_opt.step()
